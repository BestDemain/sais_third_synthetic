...
class ProteinDiffusionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 创建填充后的标签张量 - 转换为one-hot编码
        label_seq = self.labels[idx][:seq_len]
        label = torch.zeros(self.max_seq_len, 2)
...
def load_data(data_path, train_rate=0.7, valid_rate=0.2, use_all_for_train=False):
    ...
    # 使用相同的最大长度创建数据集
    train_dataset = ProteinDiffusionDataset(train_data_dicts, max_seq_len)
    valid_dataset = ProteinDiffusionDataset(valid_data_dicts, max_seq_len)
    test_dataset = ProteinDiffusionDataset(test_data_dicts, max_seq_len)
    
    return train_dataset, valid_dataset, test_dataset

def create_dataloaders(train_dataset, valid_dataset, test_dataset, batch_size=4, num_workers=4):
    """创建数据加载器
...
def convert_logits_to_labels(logits):
    """将模型输出的logits转换为标签
